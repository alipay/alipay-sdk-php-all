...
```php
query($billType, $billDate, $smid, $secure): \Alipay\OpenAPISDK\Model\AlipayDataDataserviceBillDownloadurlQueryResponseModel
```
...
try {
    $result = $apiInstance->query($billType, $billDate, $smid, $secure);
    print_r($result);
} catch (Exception $e) {
