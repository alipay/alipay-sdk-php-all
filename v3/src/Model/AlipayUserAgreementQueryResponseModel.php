...
class AlipayUserAgreementQueryResponseModel implements ModelInterface, ArrayAccess, \JsonSerializable
{
     ...
     * Sets creditAuthMode
     *
     * @param string|null $creditAuthMode 授信模式，取值：DEDUCT_HUAZHI-花芝GO。目前只在花芝代扣（即花芝go）协议时才会返回
     *
     * @return self
     */
     ...
     *
     * @param string|null $externalAgreementNo 代扣协议中标示用户的唯一签约号(确保在商户系统中唯一)
     *
     * @return self
     */
     ...
     *
     * @param string|null $externalLogonId 外部登录Id
     *
     * @return self
     */
     ...
     * Sets pricipalType
     *
     * @param string|null $pricipalType 签约主体类型。  CARD:支付宝账号  CUSTOMER:支付宝用户
     *
     * @return self
     */
     ...
     * Sets principalId
     *
     * @param string|null $principalId 签约主体标识。  当principal_type为CARD 时，该字段为支付宝用户号;  当principal_type为 CUSTOMER 时，该字段为支付宝用户标识。一个用户 可能有多个支付宝账号，即多个支付宝用户号，但只有一个是支付宝用户标识。  一个支付宝账号对应一个支付宝唯一用户号(以2088开头的16位纯数字组成)。
     *
     * @return self
     */
     ...
     *
     * @param string|null $principalOpenId 签约主体标识。 当principal_type为CARD 时，该字段为支付宝用户号; 当principal_type为 CUSTOMER 时，该字段为支付宝用户标识。一个用户 可能有多个支付宝账号，即多个支付宝用户号，但只有一个是支付宝用户标识。
     *
     * @return self
     */
     ...
     * Sets thirdPartyType
     *
     * @param string|null $thirdPartyType 签约第三方主体类型。对于三方协议，表示当前用户和哪一类的第三方主体进行签约。  取值范围：  1. PARTNER（平台商户）;  2. MERCHANT（集团商户），集团下子商户可共享用户签约内容;  默认为PARTNER。
     *
     * @return self
     */
