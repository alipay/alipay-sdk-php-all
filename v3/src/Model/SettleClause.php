...
class SettleClause implements ModelInterface, ArrayAccess, \JsonSerializable
{
     ...
     *
     * @param string|null $settleAccountIdType 结算账户id类型。  当settle_account_type 为bankCard时，本参数为cardSerialNo，表示结算账户id是银行卡编号; 当settle_account_type 为alipayBalance时，本参数为userId或者loginName，其中userId表示结算账户id是支付宝唯一用户号，loginName表示结算账户id是支付宝登录号; 当 settle_account_type 为 defaultSettle 时，本参数必须为空
     *
     * @return self
     */
     ...
     *
     * @param string|null $settleEntityType 结算主体类型。  SecondMerchant：结算主体为二级商户
     *
     * @return self
     */
